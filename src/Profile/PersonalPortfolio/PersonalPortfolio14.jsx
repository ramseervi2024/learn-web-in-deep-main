...
const SystemStatus = () => (
   <div className="fixed top-24 left-8 z-50 flex flex-col gap-6 font-mono text-[9px] uppercase tracking-widest text-[#22d3ee]/40 hidden lg:flex select-none pointer-events-none">
      <div className="flex items-center gap-3 border-l-2 border-[#22d3ee]/20 pl-4 py-1">
         ...
         <span>Kernel: Personal_v1.0.0</span>
      </div>
      ...
      <div className="bg-[#030712] text-white selection:bg-[#22d3ee] selection:text-black font-sans min-h-screen relative overflow-x-hidden">
            ...
            <div className="max-w-screen-2xl mx-auto flex justify-between items-center text-left">
               <div className="flex items-center gap-8">
                  ...
                  <div className="flex flex-col">
                     ...
                     <span className="text-[9px] font-mono text-white/30 uppercase tracking-[0.2em]">PERSONAL IDENTITY NODE</span>
                  </div>
               </div>
               ...
               <nav className="hidden lg:flex items-center gap-10 text-[10px] uppercase font-bold tracking-[0.3em] text-white/80">
                  ...
                  <div className="flex items-center gap-6">
                     ...
                     <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#22d3ee] to-[#d946ef] p-[1px]">
                        <div className="w-full h-full rounded-full bg-black flex items-center justify-center text-[10px] font-black italic select-none">{data.brand.name.charAt(0)}</div>
                     </div>
                  </div>
               </nav>
         ...
         <AnimatePresence>
            {isMenuOpen && (
               ...
               >
                  <div className="flex flex-col gap-12 text-center text-left">
                     {['Services', 'Projects', 'Pricing', 'Contact'].map(link => (
                        <a
                           key={link}
         ...
         <main className="relative z-10">
               ...
               <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                  <div className="text-left relative z-10 order-2 lg:order-1">
                     ...
                     >
                        ...
                        <span className="text-[10px] font-mono uppercase tracking-[0.4em] text-[#22d3ee]">Identity Status: Active</span>
                     </motion.div>
                     <motion.h1
                     ...
                     >
                        Personal<br /> <span className="text-transparent" style={{ WebkitTextStroke: "2px rgba(255,255,255,0.6)", color: "transparent" }}>Systems.</span>
                     </motion.h1>
                     <motion.p
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
            ...
            <section id="services" className="py-20 lg:py-32 px-8 lg:px-32">
               <SectionHeading number="01" title="Expertise Modules" />
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {data.services.map((service, i) => (
                     <GlassCard key={`service-${i}`} title={`Module // 0${i + 1}`} className="hover:border-[#22d3ee]/30 transition-all hover:-translate-y-2 group">
                        ...
                        <h3 className="text-2xl font-black italic uppercase tracking-tighter text-white/90 mb-4 text-left">{service.category}</h3>
                        <p className="text-white/60 text-sm leading-relaxed mb-8 font-light text-left">{service.description}</p>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {data.pricing.map((plan, i) => (
                     <GlassCard key={`plan-${i}`} className={`p-0 relative group transition-all duration-500 ${i === 1 ? 'border-[#22d3ee]/40' : ''}`}>
                        ...
                        <div className="bg-white/[0.01] px-12 py-4 border-t border-white/5 flex justify-between items-center opacity-40 text-left">
               ...
               <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-16 items-start">
                  <div className="text-left w-full md:w-auto">
                     <div className="flex flex-col mb-10">
                        ...
                        <span className="text-[10px] font-mono text-[#22d3ee]/60 uppercase tracking-[0.5em]">Personal_Engine_v1</span>
                     </div>
                     <p className="text-white/60 text-sm max-w-sm leading-relaxed mb-10 font-light italic">
                        High-end professional identity and technological excellence.
                     </p>
                  ...
                  <div className="flex flex-col items-start md:items-end gap-12 w-full md:w-auto text-left md:text-right">
                     <div className="flex flex-col gap-2">
                        ...
                        <a href={`mailto:${data.contact.email}`} className="text-2xl md:text-4xl font-black italic text-[#22d3ee] hover:tracking-[0.05em] transition-all">{data.contact.email}</a>
                     </div>
                     <div className="flex flex-col gap-2">
                        <span className="text-[9px] font-mono text-white/20 tracking-widest uppercase">System_Geo</span>
                        <span className="text-xl font-bold italic tracking-tighter text-white/40">REMOTE // GLOBAL</span>
                     </div>
                  </div>
               </div>

               <div className="max-w-7xl mx-auto mt-24 py-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-mono uppercase tracking-[0.3em] text-white/20 text-left">
                  <div className="flex items-center gap-4">
                     <Globe size={14} className="text-[#22d3ee]/20" />
                     <span>RPS_DS // PERSONAL_v1.0</span>
                  </div>
