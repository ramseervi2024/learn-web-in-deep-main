      ...
      <div className="bg-[#030712] text-white selection:bg-[#22d3ee] selection:text-black font-sans min-h-screen relative overflow-x-hidden">
            ...
            <div className="max-w-screen-2xl mx-auto flex justify-between items-center">
               <div className="flex items-center gap-8">
                  ...
                  <div className="flex flex-col text-left">
                     ...
                     <span className="text-[9px] font-mono text-white/30 uppercase tracking-[0.2em]">{data.personal_info.title}</span>
                  </div>
               </div>
               ...
               <nav className="hidden lg:flex items-center gap-10 text-[10px] uppercase font-bold tracking-[0.3em] text-white/80">
                  ...
                  <div className="flex items-center gap-6">
                     ...
                     <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#22d3ee] to-[#d946ef] p-[1px]">
                        <div className="w-full h-full rounded-full bg-black flex items-center justify-center text-[10px] font-black italic select-none">
         ...
         <main className="relative z-10">
               ...
               <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                  <div className="text-left relative z-10 order-2 lg:order-1">
                     ...
                     >
                        ...
                        <span className="text-[10px] font-mono uppercase tracking-[0.4em] text-[#22d3ee]">Active Session</span>
                     </motion.div>
                     <motion.h1
                     ...
                     <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-wrap gap-8">
                        <button className="px-10 py-5 bg-[#22d3ee] text-black text-[11px] font-black uppercase tracking-[0.4em] rounded-sm hover:-translate-y-1 transition-all shadow-lg hover:shadow-[#22d3ee]/20 uppercase">
                           Initialize Core
                        </button>
                        <button className="px-10 py-5 border border-white/10 text-[11px] font-black uppercase tracking-[0.4em] rounded-sm hover:bg-white/5 transition-all uppercase">
                           System Stats
                        </button>
                     </motion.div>
                  </div>
            ...
            <section id="services" className="py-20 lg:py-32 px-8 lg:px-32">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {(data.services || data.core_expertise).map((service, i) => (
                     <GlassCard key={`service-${i}`} title={`Module // 0${i + 1}`} className="hover:border-[#22d3ee]/30 transition-all hover:-translate-y-2 group text-left">
                        <div className="w-16 h-16 bg-[#22d3ee]/10 rounded-xl flex items-center justify-center mb-8 border border-[#22d3ee]/10 group-hover:bg-[#22d3ee] group-hover:text-black transition-all">
                           {i % 3 === 0 ? <Layout size={32} /> : i % 3 === 1 ? <Smartphone size={32} /> : <Database size={32} />}
                        </div>
                        <h3 className="text-2xl font-black italic uppercase tracking-tighter text-white/90 mb-4">
            ...
            <section id="projects" className="py-20 lg:py-32 px-8 lg:px-32">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  {data.projects.map((project, i) => (
                     <GlassCard key={`proj-${i}`} title={`Execution_ID: ${project.name.replace(/\s/g, '_')}`} noPad className="hover:border-[#d946ef]/30 transition-all text-left">
                        <div className="grid grid-cols-1 lg:grid-cols-12">
                           ...
                           <div className="lg:col-span-7 p-10 flex flex-col justify-between text-left">
                              <div className="text-left">
                                 <h3 className="text-3xl font-black italic tracking-tighter uppercase text-white/90 mb-4">{project.name}</h3>
                                 ...
                                 <p className="text-sm text-white/70 leading-relaxed font-light mb-8">{project.description}</p>
                              </div>
                  ...
                  {data.pricing.map((plan, i) => (
                     <GlassCard key={`plan-${i}`} className={`p-0 relative group transition-all duration-500 ${i === 1 ? 'border-[#22d3ee]/40' : ''}`}>
                        ...
                        <div className="p-12 text-left">
                           <span className="text-[10px] font-mono uppercase tracking-[0.5em] text-[#22d3ee] mb-8 block italic">{plan.plan}</span>
                           ...
                           <button className={`w-full py-5 rounded-sm text-[10px] uppercase font-black tracking-[0.4em] transition-all mt-16 uppercase ${i === 1 ? 'bg-[#22d3ee] text-black shadow-lg hover:shadow-[#22d3ee]/20' : 'bg-white/5 text-white/40 border border-white/5 hover:border-white/20 hover:text-white'}`}>
                              Acquire_License
                           </button>
                        </div>
            ...
            <footer className="py-24 px-8 lg:px-32 relative border-t border-white/5">
               <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-16 items-start text-left">
                  <div className="text-left w-full md:w-auto">
                     <div className="flex flex-col mb-10 text-left">
                        <span className="text-3xl font-black italic tracking-tighter uppercase text-white/80">{data.personal_info.name}</span>
                        <span className="text-[10px] font-mono text-[#22d3ee]/60 uppercase tracking-[0.5em]">Digital_Engine_01</span>
                     </div>
                     <p className="text-white/60 text-sm max-w-sm leading-relaxed mb-10 font-light italic text-left">
                  ...
                  <div className="flex flex-col items-start md:items-end gap-12 w-full md:w-auto text-left md:text-right">
                     <div className="flex flex-col gap-2 text-left md:text-right">
                        ...
                        <a href={`mailto:${data.personal_info.email}`} className="text-2xl md:text-4xl font-black italic text-[#22d3ee] hover:tracking-[0.05em] transition-all">{data.personal_info.email}</a>
                     </div>
                     <div className="flex flex-col gap-2 text-left md:text-right">
                        <span className="text-[9px] font-mono text-white/20 tracking-widest uppercase">System_Geo</span>
                        <span className="text-xl font-bold italic tracking-tighter text-white/40">{data.personal_info.location}</span>
                     </div>
                  </div>
               </div>
               ...
               <div className="max-w-7xl mx-auto mt-24 py-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-mono uppercase tracking-[0.3em] text-white/20">
                  ...
                  <span>© {new Date().getFullYear()} {data.personal_info.name}. All Handlers Finalized.</span>
                  <div className="flex items-center gap-6">
